...
export async function GET(req: Request) {
  ...
  for (const contact of contacts) {
    const user = `Write a LinkedIn connection request and one follow-up for this person:
...
Return ONLY JSON with keys: connection_note (max 300 chars), follow_up (max 500 chars), rationale.`
    ...
    if (message) {
      await supabase.from('approvals').insert({
        ...
        summary: `[LinkedIn] ${contact.first_name ?? ''} ${contact.last_name ?? ''} at ${contact.organization ?? 'Company'}`,
  ...
  await logAgent(AGENT_NAME, 'info', 'run', `Drafted ${actions} LinkedIn messages.`, { actions })
